...
#%%
def lag_year(data:pd.DataFrame, year:Union[float, int], x_cols:list, year_col:str = "year"):
    yearly_data = data[data[year_col] == year].reset_index(drop=True)
    w = weights.Kernel.from_dataframe(yearly_data, function="gaussian", fixed=False, k=20)
    w = fill_diagonal(w, 0)
    w.transform = 'r'
    wX = weights.spatial_lag.lag_spatial(w, yearly_data[x_cols].values.astype(float))
    lagged_data = pd.DataFrame(wX, columns = [f"lag_{name}" for name in x_cols])
    combined_data = pd.concat((yearly_data, lagged_data), axis=1)
    return combined_data

def add_lags(data: pd.DataFrame, x_cols:list, year_col:str = "year"):
    lagged_bank_data = [lag_year(data, year, x_cols, year_col) for year in data[year_col].unique()]
    return pd.concat(lagged_bank_data, axis=0)
#%%
lagged_bank_data = add_lags(bank_data, ["num_commercial_banks", "num_savings_banks", "num_commercial_banks_p1000c", "comm_lic_1", "comm_lic_1_p1000c"])
